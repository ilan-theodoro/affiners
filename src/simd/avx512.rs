...
#[target_feature(enable = "avx512f")]
pub unsafe fn apply_warp_3d_f32_avx512(
...
unsafe fn process_warp_z_slice_f32_avx512(
    ...
    for oy in 0..img_h {
        ...
        while ox + 15 < img_w {
            ...
            let _wf_x_floor = _mm512_roundscale_ps::<0x01>(wf_xs);

            let mut dz_arr = [0.0f32; 16];
            let mut dy_arr = [0.0f32; 16];
            let mut dx_arr = [0.0f32; 16];
            ...
            let dy_vec = _mm512_loadu_ps(dy_arr.as_ptr());
            let dx_vec = _mm512_loadu_ps(dx_arr.as_ptr());

            let src_zs = _mm512_sub_ps(_mm512_set1_ps(oz_f), dz_vec);
            let src_ys = _mm512_sub_ps(_mm512_set1_ps(oy_f), dy_vec);
            let src_xs = _mm512_sub_ps(vx, dx_vec);

            // Clamp tiny negative values to 0 using mask operations
            let eps_neg = _mm512_set1_ps(-1e-5);
            let zero = _mm512_setzero_ps();

            let mask_z = _mm512_cmp_ps_mask::<_CMP_GT_OQ>(src_zs, eps_neg)
                & _mm512_cmp_ps_mask::<_CMP_LT_OQ>(src_zs, zero);
            let src_zs = _mm512_mask_blend_ps(mask_z, src_zs, zero);

            let mask_y = _mm512_cmp_ps_mask::<_CMP_GT_OQ>(src_ys, eps_neg)
                & _mm512_cmp_ps_mask::<_CMP_LT_OQ>(src_ys, zero);
            let src_ys = _mm512_mask_blend_ps(mask_y, src_ys, zero);

            let mask_x = _mm512_cmp_ps_mask::<_CMP_GT_OQ>(src_xs, eps_neg)
                & _mm512_cmp_ps_mask::<_CMP_LT_OQ>(src_xs, zero);
            let src_xs = _mm512_mask_blend_ps(mask_x, src_xs, zero);

            let src_z_floor = _mm512_roundscale_ps::<0x01>(src_zs);
            let src_y_floor = _mm512_roundscale_ps::<0x01>(src_ys);
            let src_x_floor = _mm512_roundscale_ps::<0x01>(src_xs);

            let fz = _mm512_sub_ps(src_zs, src_z_floor);
            let fy = _mm512_sub_ps(src_ys, src_y_floor);
            let fx = _mm512_sub_ps(src_xs, src_x_floor);
            ...
            let zi = _mm512_cvttps_epi32(src_z_floor);
            let yi = _mm512_cvttps_epi32(src_y_floor);
            let xi = _mm512_cvttps_epi32(src_x_floor);
...
/// AVX512 optimized apply_warp for f16
#[target_feature(enable = "avx512f")]
pub unsafe fn apply_warp_3d_f16_avx512(
...
/// AVX512 optimized apply_warp for u8
#[target_feature(enable = "avx512f")]
pub unsafe fn apply_warp_3d_u8_avx512(
    image: &ArrayView3<u8>,
    warp_field: &ArrayView4<f32>,
